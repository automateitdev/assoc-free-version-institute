...
const { fetchPromotionIndex, fetchStudentList, pushbackRequest } = usePromotionStore();
...
const pushBackSearchForm = reactive({});
...
async function handlepushBackSearchForm() {
    try {
        isSearching.value = true;
        if (studentSearch.value.shift_id && pushBackSearchForm.year) {
            (formData.institute_class_map_id = studentSearch.value.instituteClassMapId),
                (formData.section_id = studentSearch.value.section_id),
                (formData.shift_id = studentSearch.value.shift_id),
                (formData.academic_year_id = pushBackSearchForm.year.id);
...
const pushBackFormData = reactive({
    present_academic_year_id: pushBackSearchForm?.year?.id ?? null,
    academic_year_id: null,
    student_id: null
});
...
async function handlePushbackRequest() {
    try {
        pushBackFormData.present_academic_year_id = pushBackSearchForm?.year?.id ?? null;
        pushBackFormData.student_id = formData.student_id = selectedStudents.value.map((elem) => elem.student_id) ?? [];
        pushBackFormData.present_combination_id = selectedStudents.value.map((elem) => elem.current_combination_id) ?? [];
        const { status, message, error } = await pushbackRequest(pushBackFormData);
        if (status == 'success') {
...
<template>
    ...
    <section class="card">
        <h4 class="bold">Migration: Push back</h4>
        <form action="" @submit.prevent="handlepushBackSearchForm" class="grid grid-flow-row">
            <div class="col-12 md:col-3">
                <label for="" class="mb-1"><b>Academic Year</b></label>
                <Dropdown v-model="pushBackSearchForm.year" :options="academicYears" filter optionLabel="core_subcategory_name" placeholder="Select Academic Year" class="w-full text-capitalize" />
            </div>

            <div class="col-12 md:col-4">
                <label for="" class="mb-1"><b>Class Shift Section</b></label>
                <Dropdown v-model="pushBackSearchForm.class" :options="classShiftSectionData.classes" class="w-full capitalize" filter optionLabel="combination" placeholder="Select a Combination" @change="getCombinationID($event)">
    ...
    <section class="card">
        ...
        >
            ...
            <template #header>
                ...
                <Button label="Push back" @click="visible = true" icon="pi pi-arrow-circle-left" size="small" v-if="studentList" :disabled="!selectedStudents?.length" />
                <!-- Dialog form for promotion -->
                <Dialog v-model:visible="visible" modal header="Push Back" style="max-width: 400px">
                    <form action="" class="grid">
                        <div class="col-12">
                            <label for=""><b>Push back to academic year</b></label>
                            <Dropdown v-model="pushBackFormData.academic_year_id" :options="academicYears" filter optionLabel="core_subcategory_name" optionValue="id" placeholder="Select Academic Year" class="w-full text-capitalize" />
                        </div>
                        ...
                        <div class="col-12 flex justify-content-end gap-2">
                            ...
                            <Button type="button" label="Push Back" @click="handlePushbackRequest" size="small" :disabled="!pushBackFormData.academic_year_id"></Button>
                        </div>
                    </form>
                </Dialog>
            </template>
