...
const { classSetupIndex, loading } = storeToRefs(useAdmissionStore());
const { fetchClassSetupIndex } = useAdmissionStore();
...
async function handleForm() {
    if (formData.details.length > 0) {
        ...
        try {
            ...
            const { data } = await axios.post('/admission-startup', {
    ...
    <div>
        ...
        <div class="card mt-3">
            <DataTable
                ...
                :value="classSetupIndex.admission_startup"
                showGridlines
                paginator
                :rows="5"
            ...
            >
                ...
                <Column sortable field="category_name" header="Category" class="capitalize"></Column>
                <Column sortable field="subcategories" header="Sub Category" class="capitalize">
                    <template #body="slotProps">{{ slotProps.data.subcategories.map((subcat) => subcat.subcategory_name).join(',  ') }}</template>
                </Column>
            </DataTable>
        </div>
    </div>
